...
1. **Database Creation:** Please run `01_Schema_Setup.sql` in your local SQL Server (SSMS) environment to create the empty tables.
...
3. **Data Migration:** Run `02_Data_Ingestion.sql` to move the raw data into the official tables.
4. **Status Update:** Please confirm in the team chat once your local databases are populated so we can begin writing the metric queries.